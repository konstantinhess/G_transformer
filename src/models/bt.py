...
class BT(TimeVaryingCausalModel):
    """
    Pytorch-Lightning implementation of Biased Transformer (no G-computation)
    """

    model_type = 'bt'  # biased transformer
    possible_model_types = {'bt'}
    ...
    def __init__(self, args: DictConfig,
        ...
        elif args.model.bt.projection_horizon is not None:
            self.projection_horizon = args.model.bt.projection_horizon
    ...
    def forward(self, batch):
        ...
        # 1) train all hidden states on factual data -> Biased transformer
        hr = self.build_hr(prev_treatments, vitals, prev_outputs, static_features, active_entries)
        pred_outcomes_all_steps = torch.zeros((batch_size, time_dim,
                                              self.projection_horizon + 1, self.dim_outcome),
                                              device=self.device)

        for i in range(self.projection_horizon+1):
            pred_outcomes = self.G_comp_heads[i-1].build_outcome(hr, curr_treatments)
            pred_outcomes_all_steps[:, i:, i-1, :] = pred_outcomes[:, :pred_outcomes.shape[1]-i, :]


        if self.training:
            return pred_outcomes_all_steps, active_entries

        else:
            return pred_outcomes_all_steps, hr
    ...
    def training_step(self, batch, batch_ind, optimizer_idx=None):
        ...
        pred_factuals, active_entries = self(batch)
        outcomes = torch.zeros(pred_factuals.shape, device=self.device)
        loss = 0.0
        for i in range(self.projection_horizon+1):
            mse_loss = F.mse_loss(pred_factuals[:, :(outcomes.shape[1]-i), i, :], batch['outputs'][:, i:, :], reduction='none')
            mse_loss = (mse_loss * active_entries[:, i:, :]).sum(dim=(0,1,2)) / (active_entries[:, i:, :].sum(dim=(0,1)) * self.dim_outcome)
            loss += mse_loss
    ...
    def predict_step(self, batch, batch_idx, dataset_idx=None):
        ...
        if self.projection_horizon == 0:
            outcome_pred = outcome_pred.squeeze(-2)
    ...
    def get_normalised_n_step_rmses(self, dataset: Dataset):
        ...
        outputs_scaled = outputs_scaled[:, outputs_scaled.shape[1]-self.projection_horizon-1, self.projection_horizon, :]
