...
def main(args: DictConfig):
    """
    Training / evaluation script for GRNN (G-RNN, ablation of G-transformer)
    Args:
    ...
    if args.exp.logging:
        ...
        grnn_callbacks += [LearningRateMonitor(logging_interval='epoch')]
    ...
    args.model.grnn.projection_horizon = 0
    grnnmodel = instantiate(args.model.grnn, args, dataset_collection, _recursive_=False)  # initialize g-transformer
    if args.model.grnn.tune_hparams:
        grnnmodel.finetune(resources_per_trial=args.model.grnn.resources_per_trial)

    grnnmodel_trainer = Trainer(gpus=eval(str(args.exp.gpus)), logger=mlf_logger, max_epochs=args.exp.max_epochs,
                              callbacks=grnn_callbacks, terminate_on_nan=True,
                              gradient_clip_val=args.model.grnn.max_grad_norm)
    grnnmodel_trainer.fit(grnnmodel)
    ...
    grnnmodel_trainer.test(grnnmodel, test_dataloaders=val_dataloader)
    # gtmodel.visualize(dataset_collection.val_f, index=0, artifacts_path=artifacts_path)
    val_rmse_orig, val_rmse_all = grnnmodel.get_normalised_masked_rmse(dataset_collection.val_f)
    ...
    if hasattr(dataset_collection, 'test_cf_one_step'):  # Test one_step_counterfactual rmse
        test_rmse_orig, test_rmse_all, test_rmse_last = grnnmodel.get_normalised_masked_rmse(
        ...
        test_rmse_orig, test_rmse_all = grnnmodel.get_normalised_masked_rmse(dataset_collection.test_f)
        logger.info(f'Test normalised RMSE (all): {test_rmse_all}; '
                    f'Test normalised RMSE (orig): {test_rmse_orig}.')
        encoder_results = {
            'encoder_val_rmse_all': val_rmse_all,
    ...
    #for t in range(6, args.dataset.projection_horizon + 1):
        ...
        # ============================== Train ===========================================
        args.model.grnn.projection_horizon = t
        grnnmodel = instantiate(args.model.grnn, args, dataset_collection, _recursive_=False)
        grnnmodel_trainer = Trainer(gpus=eval(str(args.exp.gpus)), logger=mlf_logger, max_epochs=args.exp.max_epochs,
                                  callbacks=grnn_callbacks, terminate_on_nan=True,
                                  gradient_clip_val=args.model.grnn.max_grad_norm)
        grnnmodel_trainer.fit(grnnmodel)
        ...
        if hasattr(dataset_collection, 'test_cf_treatment_seq'):  # Test n_step_counterfactual rmse
            test_rmse = grnnmodel.get_normalised_n_step_rmses(dataset_collection.test_cf_treatment_seq)
        elif hasattr(dataset_collection, 'test_f_multi'):  # Test n_step_factual rmse
            test_rmse = grnnmodel.get_normalised_n_step_rmses(dataset_collection.test_f_multi)
