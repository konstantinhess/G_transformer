...
def main(args: DictConfig):
    """
    Training / evaluation script for BT (Biased Transformer)
    Args:
    ...
    if args.exp.logging:
        ...
        bt_callbacks += [LearningRateMonitor(logging_interval='epoch')]
    ...
    args.model.bt.projection_horizon = 0
    btmodel = instantiate(args.model.bt, args, dataset_collection, _recursive_=False)  # initialize g-transformer
    if args.model.bt.tune_hparams:
        btmodel.finetune(resources_per_trial=args.model.bt.resources_per_trial)

    btmodel_trainer = Trainer(gpus=eval(str(args.exp.gpus)), logger=mlf_logger, max_epochs=args.exp.max_epochs,
                              callbacks=bt_callbacks, terminate_on_nan=True,
                              gradient_clip_val=args.model.bt.max_grad_norm)
    btmodel_trainer.fit(btmodel)
    ...
    btmodel_trainer.test(btmodel, test_dataloaders=val_dataloader)
    # btmodel.visualize(dataset_collection.val_f, index=0, artifacts_path=artifacts_path)
    val_rmse_orig, val_rmse_all = btmodel.get_normalised_masked_rmse(dataset_collection.val_f)
    ...
    if hasattr(dataset_collection, 'test_cf_one_step'):  # Test one_step_counterfactual rmse
        test_rmse_orig, test_rmse_all, test_rmse_last = btmodel.get_normalised_masked_rmse(
        ...
        test_rmse_orig, test_rmse_all = btmodel.get_normalised_masked_rmse(dataset_collection.test_f)
        logger.info(f'Test normalised RMSE (all): {test_rmse_all}; '
                    f'Test normalised RMSE (orig): {test_rmse_orig}.')
        encoder_results = {
            'encoder_val_rmse_all': val_rmse_all,
    ...
    if args.exp.logging:
        mlf_logger = FilteringMlFlowLogger(filter_submodels=[], experiment_name=experiment_name,
                                           tracking_uri=args.exp.mlflow_uri, run_name=str(args.dataset.projection_horizon))

    args.model.bt.projection_horizon = args.dataset.projection_horizon # set to max projection horizon and train

    # ============================== Train ===========================================
    btmodel = instantiate(args.model.bt, args, dataset_collection, _recursive_=False)
    btmodel_trainer = Trainer(gpus=eval(str(args.exp.gpus)), logger=mlf_logger, max_epochs=args.exp.max_epochs,
                              callbacks=bt_callbacks, terminate_on_nan=True,
                              gradient_clip_val=args.model.bt.max_grad_norm)
    btmodel_trainer.fit(btmodel)
    ...
    for t in range(1, args.dataset.projection_horizon+1):
        ...
        # ============================== Test ===========================================
        btmodel.projection_horizon = t

        if hasattr(dataset_collection, 'test_cf_treatment_seq'):  # Test n_step_counterfactual rmse
            test_rmse = btmodel.get_normalised_n_step_rmses(dataset_collection.test_cf_treatment_seq)
        elif hasattr(dataset_collection, 'test_f_multi'):  # Test n_step_factual rmse
            test_rmse = btmodel.get_normalised_n_step_rmses(dataset_collection.test_f_multi)
